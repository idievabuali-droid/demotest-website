(() => {
      ...
      const getSupabaseClient = () => {
        ...
        try {
          ...
          console.error('Failed to init Supabase client for auth', e);
          return null;
        }
      };

      const isOwner = async (supabase) => {
        try {
  ...
  *  sourceId?: string,     // if this is an owner override of a base product
   ...
   */
    ...
    const normalizeVariantLabel = (label, key) => {
      ...
      if (key === 'models' || lower.includes('model')) return 'Model';
    ...
    const computeVariantCombos = (optionsMap) => {
      const entries = Object.entries(optionsMap || {}).filter(([_, arr]) => Array.isArray(arr) && arr.length);
      if (!entries.length) {
        return [{ key: INVENTORY_BASE_KEY, selection: {} }];
      }
      const combos = [];
      const traverse = (index, acc) => {
        if (index >= entries.length) {
          const key = buildVariantKey(acc);
          combos.push({ key, selection: { ...acc } });
          return;
        }
        const [dimension, values] = entries[index];
        values.forEach((val) => traverse(index + 1, { ...acc, [dimension]: val }));
      };
      traverse(0, {});
      return combos;
    };

    const buildVariantOptionsFromForm = (form, cfg) => {
      const fields = [...(cfg.must || []), ...(cfg.also || []), ...(cfg.optional || [])];
      const variants = {};
      fields.forEach((field) => {
        const value = form[field.key];
        const label = normalizeVariantLabel(field.label, field.key);
        if (field.type === 'checkbox') {
          if (value) variants[label] = ['Yes'];
          return;
        }
        if (value == null) return;
        if (Array.isArray(value)) {
          const cleaned = value.map((v) => String(v).trim()).filter(Boolean);
          if (cleaned.length) variants[label] = Array.from(new Set(cleaned));
          return;
        }
        const trimmed = String(value).trim();
        if (trimmed) variants[label] = [trimmed];
      });
      return variants;
    };
  ...
  const RAM_OPTIONS = ["3GB","4GB","6GB","8GB","12GB","16GB"];
    ...
    const ORDER_STATUS_SEQUENCE = ['open','confirmed','shipped'];
    const ORDER_STATUS_ALLOWED = ['open','confirmed','shipped','cancelled'];
  ...
  { id: "pb-20000-dual", name: "Power Bank 20,000 mAh – Dual", sku: "PB-20K-DUAL", category: "Power Banks", price: 16.5, specs: ["20,000 mAh","Dual","PD/QC"], description: "20k dual output.", image: "Изображение WhatsApp 2025-08-16 в 15.37.19_3b58c17b.jpg", variants: { Capacity: ["20,000 mAh"], Ports: ["USB‑C + USB‑A"], Standard: STANDARDS } },
    ...
    function useCart(){
      ...
      useEffect(()=>{ try{ const raw=localStorage.getItem(CART_STORAGE_KEY); if(raw){ const p=JSON.parse(raw); if(Array.isArray(p.items)) setItems(p.items); if(typeof p.notes==='string') setNotes(p.notes);} }catch{} },[]);
      useEffect(()=>{ try{ localStorage.setItem(CART_STORAGE_KEY, JSON.stringify({ items, notes })); }catch{} },[items,notes]);
      const add = (item)=> setItems(prev=>{ const i=prev.findIndex(p=> p.id===item.id && JSON.stringify(p.variants||{})===JSON.stringify(item.variants||{})); if(i>=0){ const next=[...prev]; next[i]={...next[i], qty: next[i].qty + item.qty}; return next; } return [...prev, item]; });
      const updateQty = (index, qty)=> setItems(prev=> prev.map((it,i)=> i===index? { ...it, qty: Math.max(0, Number(qty)||0)}: it));
    ...
    const normalizeOrderRow = (row) => {
      ...
      const totalCentsValue = Number(total_cents ?? totalCents ?? 0);
      ...
      const normalizedItems = items.map((item) => {
        if (!item) return null;
        const subtotal = Number(item.subtotal_cents ?? item.subtotalCents ?? ((Number(item.quantity ?? 0) || 0) * (Number(item.unit_price_cents ?? item.unitPriceCents ?? 0) || 0)));
        return {
          ...item,
          quantity: Number(item.quantity ?? 0),
          unit_price_cents: Number(item.unit_price_cents ?? item.unitPriceCents ?? 0),
      ...
      return {
        ...
        currency: currency || OWNER_DEFAULT_CURRENCY,
        metadata: metadata || {},
    ...
    const orderTimestamp = (order) => {
      if (!order) return 0;
      const raw = order.created_at || order.createdAt || order.inserted_at || order.insertedAt;
      const value = raw ? new Date(raw).getTime() : 0;
      return Number.isFinite(value) ? value : 0;
    };

    const sortOrdersByCreatedDesc = (list) => {
      const arr = Array.isArray(list) ? list : [];
      return [...arr].sort((a, b) => orderTimestamp(b) - orderTimestamp(a));
    };
    ...
    function useOwnerOrders(){
      ...
      const fetchOrders = React.useCallback(async () => {
        try {
          ...
          setOrdersError(null);
          const response = await callOwnerApi('listOrders');
          const rows = Array.isArray(response?.data) ? response.data : [];
          const normalized = rows.map(normalizeOrderRow).filter(Boolean);
          setOrders(sortOrdersByCreatedDesc(normalized));
      ...
      const updateOrderStatus = React.useCallback(async (orderId, nextStatus) => {
        if (!orderId || !nextStatus) return null;
        try {
          const response = await callOwnerApi('updateOrderStatus', { id: orderId, status: nextStatus });
          const updated = normalizeOrderRow(response?.data);
          if (updated) {
            setOrders((prev) => {
              const mapped = prev.map((order) => order.id === updated.id ? updated : order);
          ...
          if (error?.message && /404/.test(error.message)) {
            throw new Error('Order no longer exists. Refresh the list and try again.');
          }
          throw new Error(error?.message || 'Unable to update order status. Please try again.');
        }
      }, []);
    ...
    function GaugeIcon({ className }){ return React.createElement('svg', { className, fill: 'none', viewBox: '0 0 24 24', strokeWidth: 1.5, stroke: 'currentColor' }, React.createElement('path', { strokeLinecap: 'round', strokeLinejoin: 'round', d: 'M12 13.5v2.25m2.598-9.05a6.75 6.75 0 1 1-5.196 0m7.346 9.591 1.263 1.263m-9.856-1.263L6.892 17.05m.944-4.117L5.4 8.5m13.2 0-2.436 4.433' })); }
    function PlusCircleIcon({ className }){ return React.createElement('svg', { className, fill: 'none', viewBox: '0 0 24 24', strokeWidth: 1.5, stroke: 'currentColor' }, React.createElement('path', { strokeLinecap: 'round', strokeLinejoin: 'round', d: 'M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z' })); }
    function ClipboardIcon({ className }){ return React.createElement('svg', { className, fill: 'none', viewBox: '0 0 24 24', strokeWidth: 1.5, stroke: 'currentColor' }, React.createElement('path', { strokeLinecap: 'round', strokeLinejoin: 'round', d: 'M9 5.25H7.5a2.25 2.25 0 0 0-2.25 2.25v9A2.25 2.25 0 0 0 7.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9A2.25 2.25 0 0 0 16.5 5.25H15m-6 0A1.5 1.5 0 0 1 10.5 3.75h3A1.5 1.5 0 0 1 15 5.25m-6 0v.75A1.5 1.5 0 0 0 10.5 7.5h3A1.5 1.5 0 0 0 15 6V5.25' })); }
    function TruckIcon({ className }){ return React.createElement('svg', { className, fill: 'none', viewBox: '0 0 24 24', strokeWidth: 1.5, stroke: 'currentColor' }, React.createElement('path', { strokeLinecap: 'round', strokeLinejoin: 'round', d: 'M2.25 7.5h13.5v9h-13.5V7.5Zm13.5 0H18a2.25 2.25 0 0 1 1.8.9l1.95 2.6a2.25 2.25 0 0 1 .45 1.35v4.65h-2.25m-1.5 0a1.5 1.5 0 1 0-3 0m3 0H15.75m-9 0a1.5 1.5 0 1 0-3 0m3 0H3.75' })); }
    ...
    function StatusBadge({ status }){
      ...
      const styles = normalized === 'confirmed'
      ...
      return React.createElement('span', {
        className: cx('inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium capitalize', styles),
      }, ORDER_STATUS_LABELS[normalized] || normalized || 'Open');
    }

    function SummaryCard({ title, value, subtext, trend }){
      return React.createElement('div', { className: 'rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm ring-1 ring-black/5 sm:p-5' },
        React.createElement('p', { className: 'text-sm font-medium text-slate-500' }, title),
        React.createElement('p', { className: 'mt-2 text-2xl font-semibold tracking-tight text-slate-900' }, value),
        subtext && React.createElement('p', { className: 'mt-1 text-xs text-slate-500' }, subtext),
        trend && React.createElement('div', { className: 'mt-3 inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700' }, trend)
      );
    }

    function OwnerDashboardHome({ allProducts, ownerProducts, orders, loadingOrders, ordersMeta, ordersError, onNavigate, refreshOrders }){
      const activeProducts = allProducts.filter(p => !p.hidden).length;
      const overrides = ownerProducts.filter(p => p.sourceId && !p.hidden).length;
      const netNew = ownerProducts.filter(p => !p.sourceId && !p.hidden).length;
      const hidden = ownerProducts.filter(p => p.hidden).length;

      const statusCounts = orders.reduce((acc, order) => {
        const key = (order?.status || 'open').toLowerCase();
        acc[key] = (acc[key] || 0) + 1;
        return acc;
      }, {});
      const pipelineStatuses = [...ORDER_STATUS_SEQUENCE, 'cancelled'];
      const openOrders = statusCounts.open || 0;
      const confirmedOrders = statusCounts.confirmed || 0;
      const shippedOrders = statusCounts.shipped || 0;

      const revenueCents = orders
        .filter((order) => ['confirmed', 'shipped'].includes((order?.status || '').toLowerCase()))
        .reduce((sum, order) => sum + (Number(order?.total_cents) || 0), 0);
      const revenueLabel = new Intl.NumberFormat('en-GB', { style: 'currency', currency: CURRENCY }).format(revenueCents / 100);
      ...
      return React.createElement('div', { className: 'flex flex-col gap-10' },
        React.createElement('header', { className: 'flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between' },
          React.createElement('div', null,
            React.createElement('h1', { className: 'text-2xl font-semibold tracking-tight text-slate-900' }, 'Owner dashboard'),
            React.createElement('p', { className: 'mt-1 text-sm text-slate-600' }, 'Monitor catalogue health and stay on top of incoming orders.')
          )
        ),

        React.createElement('section', { className: 'grid gap-4 sm:grid-cols-2 xl:grid-cols-4' },
          React.createElement(SummaryCard, { title: 'Active SKUs', value: activeProducts, subtext: overrides ? `${overrides} overrides` : undefined }),
          React.createElement(SummaryCard, { title: 'Owner additions', value: netNew, subtext: hidden ? `${hidden} hidden` : 'Live to customers' }),
          React.createElement(SummaryCard, { title: 'Open orders', value: loadingOrders ? '—' : openOrders, subtext: confirmedOrders ? `${confirmedOrders} confirmed` : 'Awaiting confirmation' }),
          React.createElement(SummaryCard, { title: 'Fulfilled value', value: revenueLabel, subtext: shippedOrders ? `${shippedOrders} shipped` : 'No shipments yet' })
        ),

        React.createElement('section', { className: 'grid gap-6 lg:grid-cols-[minmax(0,1fr),minmax(0,1fr)]' },
          React.createElement('div', { className: 'rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm ring-1 ring-black/5' },
            React.createElement('div', { className: 'flex items-center justify-between gap-3' },
              React.createElement('div', null,
                React.createElement('h2', { className: 'text-base font-semibold text-slate-900' }, 'Order pipeline'),
                React.createElement('p', { className: 'mt-1 text-sm text-slate-500' }, 'Track each stage to keep fulfilment moving.')
              ),
              React.createElement('button', {
                type: 'button',
                onClick: refreshOrders,
                className: 'inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus-visible:outline-2 focus-visible:outline-brand',
                disabled: loadingOrders
              }, loadingOrders ? 'Refreshing…' : 'Refresh')
            ),
            ordersError ? (
              /schema cache|relation "orders"/i.test(ordersError.message || '') ?
                React.createElement('div', { className: 'mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900' },
                  React.createElement('p', { className: 'font-medium' }, 'Orders table not detected'),
                  React.createElement('p', { className: 'mt-1' }, 'Set up the orders tables & policies in Supabase, then refresh to start tracking customer purchase history.')
                )
              :
                React.createElement('div', { className: 'mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-900' },
                  React.createElement('p', { className: 'font-medium' }, 'Orders could not be loaded'),
                  React.createElement('p', { className: 'mt-1' }, ordersError.message || 'Check your connection or Supabase policies and try again.')
                )
            ) : ordersMeta?.missingOrdersTable ? (
              React.createElement('div', { className: 'mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900' },
                React.createElement('p', { className: 'font-medium' }, 'Orders table not detected'),
                React.createElement('p', { className: 'mt-1' }, 'Set up the orders tables & policies in Supabase, then refresh to start tracking customer purchase history.')
              )
            ) : (
              React.createElement('dl', { className: 'mt-6 grid grid-cols-1 gap-4 sm:grid-cols-4' },
                pipelineStatuses.map((stage) => React.createElement('div', {
                  key: stage,
                  className: cx('rounded-xl border px-4 py-3 text-center flex flex-col items-center justify-center gap-1',
                    stage === 'open' ? 'border-amber-200 bg-amber-50 text-amber-800' :
                    stage === 'confirmed' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' :
                    stage === 'shipped' ? 'border-sky-200 bg-sky-50 text-sky-700' :
                    'border-rose-200 bg-rose-50 text-rose-700')
                },
                  React.createElement('dt', { className: 'text-[11px] font-medium uppercase tracking-wide' }, ORDER_STATUS_LABELS[stage] || stage),
                  React.createElement('dd', { className: 'text-2xl font-semibold leading-none' }, loadingOrders ? '—' : (statusCounts[stage] || 0))
                ))
              )
            )
          ),
          React.createElement('div', { className: 'rounded-2xl border border-slate-200 bg-white/80 p-6 shadow-sm ring-1 ring-black/5' },
            React.createElement('div', { className: 'flex items-center justify-between gap-3' },
              React.createElement('div', null,
                React.createElement('h2', { className: 'text-base font-semibold text-slate-900' }, 'Recent orders'),
                React.createElement('p', { className: 'mt-1 text-sm text-slate-500' }, recentOrders.length ? 'Latest activity from verified customers.' : 'No orders yet — your list will appear here.')
              ),
              React.createElement('button', {
                type: 'button',
            ...
            recentOrders.length > 0 && React.createElement('ul', { className: 'mt-5 space-y-3' },
              recentOrders.map((order) => React.createElement('li', {
                key: order.id,
                className: 'rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm'
              },
                React.createElement('div', { className: 'flex items-start justify-between gap-3' },
                  React.createElement('div', null,
                    React.createElement('p', { className: 'text-sm font-medium text-slate-900' }, order.customer_name || order.customer_email || 'Customer'),
                    React.createElement('p', { className: 'text-xs text-slate-500' }, order.customer_email || 'Email pending')
                  ),
                  React.createElement(StatusBadge, { status: order.status })
                ),
                React.createElement('div', { className: 'mt-3 flex items-center justify-between text-xs text-slate-500' },
                  React.createElement('span', null, new Intl.DateTimeFormat('en-GB', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(order.created_at || order.createdAt || Date.now()))),
                  React.createElement('span', { className: 'font-medium text-slate-700' }, new Intl.NumberFormat('en-GB', { style: 'currency', currency: CURRENCY }).format((Number(order.total_cents) || 0) / 100))
                )
              ))
            )
          )
    ...
    function OwnerOrdersBoard({ orders, loading, error, meta, onRefresh, onStatusChange, onNotify }){
      ...
      const statuses = useMemo(() => Array.from(new Set([...ORDER_STATUS_SEQUENCE, 'cancelled', 'all'])), []);

      const filteredOrders = useMemo(() => {
        if (activeStatus === 'all') return orders;
        return orders.filter((order) => (order?.status || 'open').toLowerCase() === activeStatus);
      }, [orders, activeStatus]);
      ...
      const currencyFormatter = useMemo(() => new Intl.NumberFormat('en-GB', { style: 'currency', currency: CURRENCY }), []);
      ...
      const handleStatusAdvance = async (orderId, nextStatus) => {
        ...
        if (!ORDER_STATUS_ALLOWED.includes(normalizedNextStatus)) {
          onNotify?.('error', `Unsupported status transition: ${nextStatus}`);
          return;
        }
        try {
          setUpdating(true);
          const updated = await onStatusChange?.(orderId, normalizedNextStatus);
          if (updated) {
            const label = ORDER_STATUS_LABELS[normalizedNextStatus] || normalizedNextStatus;
            onNotify?.('success', `Order status updated to ${label}.`);
          }
        } catch (err) {
          console.error('Failed to update order status', err);
          onNotify?.('error', err?.message || 'Unable to update order status right now.');
        } finally {
          setUpdating(false);
        }
      };

      if (meta?.missingOrdersTable) {
        return React.createElement('div', { className: 'rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900 shadow-sm' },
          React.createElement('h2', { className: 'text-base font-semibold text-amber-900' }, 'Orders table not detected'),
          React.createElement('p', { className: 'mt-2' }, 'Create the orders and order_items tables in Supabase, apply RLS policies, then refresh this view to start tracking customer history.'),
          React.createElement('div', { className: 'mt-4 flex flex-wrap gap-2 text-xs' },
            React.createElement('span', { className: 'rounded-full border border-amber-300 bg-white px-3 py-1 text-amber-700' }, 'orders'),
            React.createElement('span', { className: 'rounded-full border border-amber-300 bg-white px-3 py-1 text-amber-700' }, 'order_items')
          ),
          React.createElement('button', {
            type: 'button',
            onClick: onRefresh,
            className: 'mt-4 inline-flex items-center gap-2 rounded-full border border-amber-300 bg-white px-4 py-2 text-xs font-medium text-amber-800 hover:bg-amber-100 focus:outline-none focus-visible:outline-2 focus-visible:outline-amber-500'
          }, 'Refresh')
        );
      }
      ...
      if (error) {
        ...
        if (/schema cache|relation "orders"/i.test(message)) {
          return React.createElement('div', { className: 'rounded-2xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-900 shadow-sm' },
            React.createElement('h2', { className: 'text-base font-semibold text-amber-900' }, 'Orders table not detected'),
            React.createElement('p', { className: 'mt-2' }, 'Create the orders and order_items tables in Supabase, apply RLS policies, then refresh this view to start tracking customer history.'),
            React.createElement('div', { className: 'mt-4 flex flex-wrap gap-2 text-xs' },
        ...
        return React.createElement('div', { className: 'rounded-2xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-900 shadow-sm' },
          React.createElement('h2', { className: 'text-base font-semibold text-rose-900' }, 'Unable to load orders'),
          React.createElement('p', { className: 'mt-2' }, error.message || 'Check your connection or Supabase permissions and try again.'),
          React.createElement('button', {
            type: 'button',
            onClick: onRefresh,
            className: 'mt-4 inline-flex items-center gap-2 rounded-full border border-rose-300 bg-white px-4 py-2 text-xs font-medium text-rose-700 hover:bg-rose-100 focus:outline-none focus-visible:outline-2 focus-visible:outline-rose-500'
          }, 'Retry')
        );
      }

      return React.createElement('div', { className: 'flex flex-col gap-6' },
        React.createElement('header', { className: 'flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between' },
          React.createElement('div', null,
            React.createElement('h2', { className: 'text-xl font-semibold text-slate-900' }, 'Orders pipeline'),
            React.createElement('p', { className: 'text-sm text-slate-600' }, 'Move orders from open to shipped, while keeping a full audit trail.')
          ),
          React.createElement('div', { className: 'flex flex-wrap gap-2 text-xs text-slate-500' },
            React.createElement('span', { className: 'rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-amber-700' }, `Open: ${statusCounts.open || 0}`),
            React.createElement('span', { className: 'rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-emerald-700' }, `Confirmed: ${statusCounts.confirmed || 0}`),
            React.createElement('span', { className: 'rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-sky-700' }, `Shipped: ${statusCounts.shipped || 0}`),
            React.createElement('button', {
              type: 'button',
              onClick: onRefresh,
              className: 'inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-3 py-1 text-slate-700 hover:bg-slate-50 focus:outline-none focus-visible:outline-2 focus-visible:outline-brand'
            }, loading ? 'Refreshing…' : 'Refresh')
          )
        ),

        React.createElement('div', { className: 'flex flex-wrap gap-2' },
          statuses.map((status) => React.createElement('button', {
            key: status,
            type: 'button',
            onClick: () => setActiveStatus(status),
            className: cx(
              'rounded-full border px-3 py-1.5 text-xs font-medium transition focus:outline-none focus-visible:outline-2 focus-visible:outline-brand',
              activeStatus === status
                ? 'border-brand bg-[var(--brand-soft-strong)] text-brand shadow-sm'
                : 'border-slate-300 bg-white text-slate-600 hover:bg-slate-50'
            )
          }, status === 'all' ? 'All' : ORDER_STATUS_LABELS[status] || status))
        ),

        loading && !orders.length ? React.createElement('div', { className: 'flex items-center justify-center rounded-2xl border border-slate-200 bg-white/80 p-10 text-slate-500' }, 'Loading orders…') :
        !filteredOrders.length ? React.createElement('div', { className: 'rounded-2xl border border-slate-200 bg-white/80 p-8 text-sm text-slate-500' }, 'No orders in this stage yet.') :
        React.createElement('div', { className: 'grid gap-6 lg:grid-cols-[minmax(0,1fr),minmax(0,360px)]' },
          React.createElement('div', { className: 'space-y-3' },
            filteredOrders.map((order) => React.createElement('button', {
              key: order.id,
              type: 'button',
              onClick: () => setSelectedId(order.id),
              className: cx(
                'w-full rounded-2xl border px-4 py-3 text-left shadow-sm transition focus:outline-none focus-visible:outline-2 focus-visible:outline-brand',
                order.id === selectedId ? 'border-brand bg-[var(--brand-soft-strong)]/70 text-slate-900 shadow-md' : 'border-slate-200 bg-white hover:border-slate-300'
              )
            },
              React.createElement('div', { className: 'flex items-start justify-between gap-3' },
                React.createElement('div', null,
                  React.createElement('p', { className: 'text-sm font-semibold text-slate-900' }, order.customer_name || 'Customer'),
                  React.createElement('p', { className: 'text-xs text-slate-500' }, order.customer_email || 'Email pending')
                ),
                React.createElement(StatusBadge, { status: order.status })
              ),
              React.createElement('div', { className: 'mt-3 flex items-center justify-between text-xs text-slate-500' },
                React.createElement('span', null, formatDateTime(order.created_at || order.createdAt)),
                React.createElement('span', { className: 'font-medium text-slate-700' }, formatCurrency(order.total_cents))
              )
            ))
          ),
          React.createElement('aside', { className: 'rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm' },
            selectedOrder ? React.createElement('div', { className: 'flex flex-col gap-4' },
              React.createElement('div', { className: 'flex items-start justify-between gap-3' },
                React.createElement('div', null,
                  React.createElement('h3', { className: 'text-base font-semibold text-slate-900' }, selectedOrder.customer_name || 'Customer'),
                  React.createElement('p', { className: 'text-xs text-slate-500' }, selectedOrder.customer_email || 'Email pending')
                ),
                React.createElement(StatusBadge, { status: selectedOrder.status })
              ),
              React.createElement('dl', { className: 'grid grid-cols-2 gap-3 text-xs text-slate-500' },
                React.createElement('div', null,
                  React.createElement('dt', { className: 'font-medium uppercase tracking-wide text-slate-600' }, 'Placed'),
                  React.createElement('dd', { className: 'mt-1 text-slate-700' }, formatDateTime(selectedOrder.created_at || selectedOrder.createdAt))
                ),
                React.createElement('div', null,
                  React.createElement('dt', { className: 'font-medium uppercase tracking-wide text-slate-600' }, 'Total'),
                  React.createElement('dd', { className: 'mt-1 text-slate-700 font-semibold' }, formatCurrency(selectedOrder.total_cents))
                ),
                selectedOrder.updated_at && React.createElement('div', { className: 'col-span-2' },
                  React.createElement('dt', { className: 'font-medium uppercase tracking-wide text-slate-600' }, 'Updated'),
                  React.createElement('dd', { className: 'mt-1 text-slate-700' }, formatDateTime(selectedOrder.updated_at))
                )
              ),
              selectedOrder.order_items && selectedOrder.order_items.length > 0 && React.createElement('div', null,
                React.createElement('h4', { className: 'text-sm font-semibold text-slate-800' }, 'Items'),
                React.createElement('ul', { className: 'mt-3 space-y-3' },
                  selectedOrder.order_items.map((item) => React.createElement('li', { key: item.id || `${item.product_id}-${item.sku}` },
                    React.createElement('div', { className: 'flex items-start justify-between gap-3 text-sm text-slate-700' },
                      React.createElement('div', null,
                        React.createElement('p', { className: 'font-medium text-slate-900' }, item.name || 'Item'),
                        item.sku && React.createElement('p', { className: 'text-xs text-slate-500' }, 'SKU: ', item.sku)
                      ),
                      React.createElement('span', { className: 'text-xs text-slate-500' }, `×${item.quantity || 0}`)
                    ),
                    item.variants && Object.keys(item.variants).length > 0 && React.createElement('div', { className: 'mt-1 flex flex-wrap gap-1 text-xs text-slate-600' },
                      Object.entries(item.variants).map(([key, rawValue]) => {
                        const value = Array.isArray(rawValue) ? rawValue.join(', ') : rawValue;
                        const isColor = /color/i.test(key);
                        return React.createElement('span', {
                          key: key,
                          className: cx('inline-flex items-center gap-1 rounded-full border px-2 py-0.5',
                            isColor ? 'border-slate-300 bg-slate-50' : 'border-slate-200 bg-white')
                        },
                          isColor ? React.createElement(ColorDot, { name: value }) : null,
                          `${key}: ${value}`
                        );
                      })
                    ),
                    React.createElement('div', { className: 'mt-1 flex items-center justify-between text-xs text-slate-500' },
                      React.createElement('span', null, formatCurrency(item.unit_price_cents)),
                      React.createElement('span', { className: 'font-medium text-slate-700' }, formatCurrency(item.subtotal_cents))
                    )
                  ))
                )
              ),
              (selectedOrder.note || selectedOrder.metadata?.notes) && React.createElement('div', { className: 'rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600' },
                selectedOrder.note || selectedOrder.metadata?.notes
              ),
              React.createElement('div', { className: 'flex flex-wrap gap-2' },
                (() => {
                  const status = (selectedOrder.status || 'open').toLowerCase();
                  const actions = [];
                  if (status === 'open') {
                    actions.push({ label: 'Mark as confirmed', next: 'confirmed' });
                    actions.push({ label: 'Cancel order', next: 'cancelled', variant: 'danger' });
                  }
                  if (status === 'confirmed') {
                    actions.push({ label: 'Mark as shipped', next: 'shipped' });
                    actions.push({ label: 'Cancel order', next: 'cancelled', variant: 'danger' });
                  }
                  return actions.map((action) => React.createElement('button', {
                    key: action.next,
                    type: 'button',
                    onClick: () => handleStatusAdvance(selectedOrder.id, action.next),
                    disabled: updating,
                    className: cx(
                      'inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-medium shadow-sm focus:outline-none focus-visible:outline-2 focus-visible:outline-brand',
            ...
            ) : React.createElement('p', { className: 'text-sm text-slate-500' }, filteredOrders.length ? 'Select an order to see its breakdown.' : 'No orders in this stage yet.')
          )
        )
      );
    }
      ...
      return React.createElement('main', { id: 'main', className: 'mx-auto w-full max-w-7xl px-4 pb-16 pt-6 sm:px-6 lg:px-8' },
        React.createElement('div', { className: 'grid gap-6 lg:grid-cols-[minmax(200px,240px),minmax(0,1fr)]' },
          ...
          React.createElement('section', { className: 'min-h-[480px]' },
            current?.element || React.createElement('p', { className: 'text-sm text-slate-500' }, 'Select a section to begin.')
          )
        )
      );
    }
  ...
  function Header({ onOpenCart, search, setSearch, ownerMode, setOwnerMode, cartQty, connectionStatus, allowOwnerToggle }){
    ...
    function CategoryChips({ active, setActive }){
      ...
      const containerClasses = cx(
        'relative -mx-4 overflow-x-auto px-4 chip-scroll-fade',
        fade.left && 'show-left',
      ...
      return React.createElement(React.Fragment, null,
        React.createElement('div', { className: 'sticky top-[var(--header-offset,64px)] z-30 border-b border-[var(--surface-border)] bg-[rgba(246,247,251,0.85)] backdrop-blur-lg' },
          React.createElement('div', { className: 'mx-auto max-w-7xl px-4 sm:px-6 lg:px-8', 'aria-label': 'Categories' },
            React.createElement('div', { ref: containerRef, className: containerClasses },
  ...
  const [imagesData, setImagesData] = useState([]); // array of data URLs
      ...
      return React.createElement(
        ...
        React.createElement('div', { className: 'space-y-6 rounded-3xl border border-[var(--surface-border)] bg-white/95 p-6 sm:p-8 shadow-soft-xl' },
          ...
          React.createElement('div', { className: 'space-y-6 rounded-3xl border border-[var(--surface-border)] bg-white/95 p-6 shadow-soft-xl sm:p-8' },
            React.createElement('div', { className: 'sticky top-[calc(var(--header-offset,64px)+12px)] z-10 rounded-2xl border border-[var(--surface-border)] bg-white/90 px-4 py-3 shadow-sm backdrop-blur' },
              React.createElement('div', { className: 'flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between' },
                ...
                React.createElement('div', { className: 'flex items-center gap-2' },
                  React.createElement('label', { className: 'relative flex-1 sm:w-auto' },
                    React.createElement('span', { className: 'sr-only' }, 'Search'),
                    React.createElement('input', {
                      value: search,
                      onChange: (e)=> setSearch(e.target.value),
                      placeholder: 'Search…',
                      type: 'search',
                      className: 'w-full rounded-full border border-[var(--surface-border)] bg-white px-3.5 py-2 text-sm font-medium text-slate-600 shadow-inner placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent'
                    }),
                    React.createElement(SearchIcon, { className: 'pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400' })
                  ),
                  React.createElement('button', { onClick: onClose, className: 'rounded-full border border-[var(--surface-border)] bg-white px-3.5 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-white/90 focus:outline-none focus-visible:outline-2 focus-visible:outline-brand' }, 'Back')
                )
              )
            ),
    ...
    function App(){
      ...
      // expose owner manager setter globally for legacy buttons
      useEffect(() => {
        window.__setOwnerManage = (open) => {
          setOwnerSection(open ? 'manage' : 'dashboard');
        };
        return () => {
          window.__setOwnerManage = undefined;
        };
      }, []);
      ...
      const ownerDashboardElement = useMemo(() => React.createElement(OwnerDashboardHome, {
        allProducts,
        ownerProducts,
        orders,
        loadingOrders,
        ...
        refreshOrders,
      }), [allProducts, ownerProducts, orders, loadingOrders, ordersMeta, ordersError, refreshOrders]);

      const ownerViews = useMemo(() => [
        {
          id: 'dashboard',
          label: 'Dashboard',
          description: 'Key metrics & recents',
          icon: GaugeIcon,
          element: ownerDashboardElement,
        },
        {
          id: 'add',
          label: 'Add product',
          ...
          icon: PlusCircleIcon,
          element: React.createElement(OwnerPage, {
            addProduct,
            onNotify: pushNotice,
            onNavigate: setOwnerSection,
            catalogConfig,
        ...
        },
        {
          ...
          description: 'Edit overrides & hide base items',
          icon: ClipboardIcon,
          element: React.createElement(OwnerManager, {
            ...
            onDelete: (id) => deleteOwnerProduct(id),
            onSave: saveEdit,
          }),
        },
        {
          id: 'orders',
          label: 'Orders',
          description: 'Track open, confirmed & shipped',
          icon: TruckIcon,
          element: React.createElement(OwnerOrdersBoard, {
            orders,
  ...
  React.createElement(Header, { onOpenCart: ()=> { lastCatalogScrollRef.current = window.scrollY || 0; setCartOpen(true); }, search, setSearch, ownerMode, setOwnerMode: toggleOwnerMode, cartQty, connectionStatus, allowOwnerToggle: OWNER_TOGGLE_ENABLED }),
        ownerMode ? (
          React.createElement(OwnerConsole, { views: ownerViews, section: ownerSection, onSectionChange: setOwnerSection })
        ) : (
        ...
        React.createElement(CartDrawer, { open: cartOpen, onClose: ()=> setCartOpen(false), cart })
      );
    }
    ...
    root.render(React.createElement(App));
      ...
      const showAuthScreen = (supabase) => {
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(15,23,42,0.9);z-index:9999;padding:24px;';
        overlay.innerHTML = `
        ...
        const signOutBtn = overlay.querySelector('[data-signout]');
        emailInput.focus();
        ...
        form.addEventListener('submit', async (event) => {
          ...
          try {
            ...
            const ownerOk = await isOwner(supabase);
            if (ownerOk === true) {
              overlay.remove();
              mountApp();
              return;
            ...
            setStatus(ownerOk === false
              ? `Signed in, but not an owner. User ID: ${uid || '(unknown)'}`
              : 'Signed in, but owner check failed. Ensure public.is_owner() exists and you are listed in public.owner_users.');
          } catch (e) {
